...
namespace CryptoToolkitUnitTests.SymKey
{
    public class BlowfishTests
    {
        ...
        {
            using (FileStream fsDat = StreamHelper.GetFileStreamOpen(@"data\SymKey\blowfish_data.dat"))
            {
                using (FileStream fsEnc = StreamHelper.GetFileStreamOpen(@"data\SymKey\blowfish_enc.dat"))
                {
